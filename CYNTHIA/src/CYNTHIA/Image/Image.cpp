...
namespace Cynthia
{
	...
	template < typename T >
	Image< T > & Image< T >::blur ( double sigma )
	{
		return *this;
	}

	template < typename T >
	Image< T > Image< T >::getNorm ( )
	{
		...
		for ( int i = 0 ; i < this->m_height ; i++ )
		{
			for ( int j = 0 ; j < this->m_width ; j++ )
			{
				Vector<float> temp = static_cast<Vector< float >>(gradientNorm.m_image[ i ][ j ]);
				temp.resize(1);
				temp[0] = this->m_image(i,j).norm();
//				gradientNorm.m_image[i][j] = temp;
				static_cast<Vector<float>>(gradientNorm.m_image[i][j]);
			}
		}
		return gradientNorm;
	}
	...
	template < typename T >
	void Image< T >::onUpdate ( )
	{

	}

	template < typename T >
	Image< T > & Image< T >::normalize ( int min , int max )
	{
		for ( int i = 0 ; i < this->m_height ; i++ )
		{
			for ( int j = 0 ; j < this->m_width ; j++ )
